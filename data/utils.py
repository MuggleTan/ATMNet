...
def random_rotate(images, max_rotation_angle, interpolation, crop_valid=False):
    ...
    if crop_valid:
        ...
        crop_params = np.floor(np.asarray(rotated[0].shape[1:3]) - 2. *
                               (np.sin(np.abs(angle * np.pi / 180.)) * np.asarray(images[0].shape[1:3][::-1]))).astype(
            int)
        return [F.center_crop(image, crop_params) for image in rotated]
    else:
        return [F.rotate(image, angle, interpolation, ROTATION_EXPAND, ROTATION_CENTER, ROTATION_FILL) for image in
                images]
